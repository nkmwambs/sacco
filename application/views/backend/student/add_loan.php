...
$rec_cond = "loans_id=".$_id."";

//echo $_id;

$rec = $this->db->where($rec_cond)->get('loans')->result_object();

$available_shares = $this->sacco_model->net_shares($this->session->userdata('login_user_id'));
...
$loan_limit = $this->sacco_model->loan_limit($row->loan_type,$this->session->userdata('login_user_id'));

$loans_id = $row->loans_id;

$loan_status = ucfirst($row->status);

$position_id = $this->db->get_where('student',array('student_id'=>$row->member_id))->row()->position_in_sacco;

$position = $this->db->get_where('position_in_sacco',array('position_in_sacco_id'=>$position_id))->row()->name;

$employed_as = $this->db->get_where('student',array('student_id'=>$row->member_id))->row()->position_employed;
...
<div class="row">
	...
	<div class="well well-sm">
		...
		<?php echo form_open(base_url() . 'index.php?student/loan/save/' , array('id'=>'frmwizard','class' => 'form-wizard validate', 'enctype' => 'multipart/form-data'));?>
			...
			<ul>
				...
				<li id="id-tab2-7">
					<a href="#tab2-7" data-toggle="tab"><span>7</span><?php echo get_phrase('loan_status');?></a>
				</li>							
			</ul>
			...
			<div class="tab-content">
				<div class="tab-pane" id="tab2-1">
					...
					<div class="row">
						...
						<div class="col-md-5">
							<div class="form-group">
								...
								<?php
									$rst = $this->db->get_where("student",array("student_id"=>$this->session->student_id))->row();
								?>
					...
					<div class="row">
						...
						<div class="col-md-5">
							<div class="form-group">
								<label class="control-label" for="payroll_number"><?php echo get_phrase('payroll_number');?></label>
								<input class="form-control" readonly="readonly" name="payroll_number" id="payroll_number" data-validate="required" value="<?php echo $rst->payroll_number;?>"/>
							</div>
						</div>
						...
						<div class="col-md-5 col-md-offset-2">
							<div class="form-group">
								...
								<input class="form-control" name="position_employed" id="position_employed" data-validate="required"  value="<?= $employed_as;?>" readonly/>
							</div>
						</div>
					...
					<div class="row">
						...
						<div class="col-md-5 col-md-offset-2">
							<div class="form-group">
								<label class="control-label" for="terms_of_service"><?php echo get_phrase("terms_of_service");?></label>
								<input class="form-control" readonly="readonly" name="terms_of_service" id="terms_of_service" value="<?php echo $this->crud_model->get_type_name_by_id('terms_of_service',$rst->terms_of_service);?>"/>
							</div>
						</div>
					...
					<div class="row">
						...
						<div class="col-md-5 col-md-offset-2">
							<div class="form-group">
								<label class="control-label" for="position_in_sacco"><?php echo get_phrase("position_in_sacco");?></label>
								<input class="form-control" name="position_in_sacco" id="position_in_sacco" value="<?= $position;?>" readonly/>
							</div>
						</div>
					...
					<div class="row">
						<div class="col-md-10">	
							<div class="form-group">
									<label class="col-sm-3 control-label">Attach 3 recent payslips scanned in one file</label>
									
									<div class="col-sm-5">
										
										<input type="file" name="payslip" id="payslip" class="form-control file2 inline btn btn-primary"   data-label="<i class='glyphicon glyphicon-circle-arrow-up'></i> &nbsp;Browse Files" />
					...
					<div class="row">
							...
							<div class="panel panel-primary">
								<div class="panel-heading">
									<div class="panel-title"><?= get_phrase('uploaded_payslips')?></div>
								</div>
							
							<?php $map = directory_map('./uploads/payslip/'.$_id, FALSE, TRUE);
								//print_r($map);
							...
							?>
								<div class="panel-body">	
									<div class="well well-sm">
										<div>
											<a href="#" onclick="confirm_action('<?php echo base_url();?>index.php?student/delete_payslip/<?= $_id;?>/<?= $map[0];?>');" class="btn btn-icon btn-red btn-xs"><i class="fa fa-trash-o"></i><?= get_phrase('delete')?></a>
											<a href="#" onclick="confirm_action('<?php echo base_url();?>index.php?student/force_download/<?= $_id;?>/<?= $map[0];?>');"><?= $map[0];?></a>
										</div>
									</div>
								</div>
							<?php
					...
					<div class="row">
						<table class="table table-bordered datatable example">
							...
							<tbody>
								...
								?>
								<tr>
									<td><div onclick="confirm_action('<?php echo base_url();?>index.php?student/update_loan/free_guarantor/<?php echo $r->member_id;?>/<?php echo $loans_id;?>');" class="btn btn-icon btn-info">
					...
					<div class="row">
	                    ...
	                    <ul class="dropdown-menu dropdown-default pull-right" role="menu">
	                       ...
	                       <!-- Guarantee Request Link -->
	                        <li>
	                            <a href="#" onclick="confirm_action('<?php echo base_url();?>index.php?student/update_loan/create_gurantor/<?php echo $rws['member_id'];?>/<?php echo $loans_id;?>');">
				...
				<div class="tab-pane" id="tab2-7">
					...
					<div class="row">
						<div class="col-xs-5">
							...
							<textarea class="form-control" cols="12" rows="5" readonly="readonly"><?= $deferred_comment->comment;?></textarea>
						</div>
						...
						<div class="col-xs-5 col-xs-offset-2">
							...
							?>
							<textarea class="form-control" cols="12" rows="5" readonly="readonly"><?php echo $declined_comment->comment;?></textarea>
						</div>
...
$('#btn-save').click(function(){ 
	...
	
	{
		...
		$('#status').on('change',function(){
			...
			confirm_action('<?php echo base_url();?>index.php?student/update_loan/submit/<?php echo $loans_id;?>');
	...
	//Get Scheduled Repayments
		$('#repayment_period,#principle,#loan_type').change(function(){
			...
			var sched_pay = (princ * intr / (1 - (Math.pow(1/(1 + intr), term)))).toFixed(2);
			if(term==='0'||term===''){sched_pay=0;}
		...
		$("#loan_type").change(function(){
			...
			var url2 = '<?php echo base_url();?>index.php?student/get_loan_limit/'+loan;
			$.ajax({
			url: url,
		...
		//Get loan Maximum limit
				
			$.ajax({
				url:url2,
				success:function(rst){
					jQuery('#loan_limit').val(rst);
				}
			});
